...
- The ``key`` is the official W3C CSS property name (also see datalibrary.DataLibrary.property_names).

- The ``value`` is a ``set()`` of custom string aliases.

- When adding a new alias it must end with a ``'-'``. Specifically, if ``'azi'`` is used without the ``'-'``;
  then blowdrycss assumes that ``'azi'`` is a valid CSS property (*which it is not*).

- An alias must be unique across all defined aliases. Any alias that clashes with an alias in this dictionary
  or the dictionary auto-generated by ``initialize_property_alias_dict()`` is removed, and become unusable.
